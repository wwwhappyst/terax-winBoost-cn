...
- `main` 只作为上游同步基线，不承载个人功能提交；作者更新先通过 GitHub Fork 的同步功能进入 `origin/main`，再快进本地 `main`。
- 个人功能按子系统拆成独立提交，禁止把汉化、快捷键和通知混入同一个提交。
- 同步上游后，将最新 `main` 合并到已推送的个人功能分支，避免改写个人分支历史；合并前先做虚拟三方合并，确认冲突范围。
...
- 个人功能不主动新增前端、Rust 或系统依赖，不因个人改动手工修改锁文件；上游版本同步允许按上游提交更新锁文件。
