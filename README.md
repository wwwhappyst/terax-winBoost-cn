...
当前个人包基于上游 `0.8.5`，本仓库小改版号 `0.10`。Windows 安装包：`Terax_0.8.5-0.10_x64-setup.exe`。

### 主要更新

- **Agent 通知更完整**：新增 Kimi Code Hook；Windows 优先经命名管道投递事件（ConPTY 仅作回退）；完善 Grok / Claude 进程树校验，减少串台误报。
- **任务完成可见反馈**：Agent `finished` 后，对应终端分屏与所属页签以外框主题色慢闪，鼠标移入确认后停止。
- **终端本地体验**：改进 IME 候选框定位、焦点跟随鼠标、输入焦点恢复；页签标题与窗口标题显示更清晰。
- **修复外链卡死**：网页链接与终端 OSC 8 超链接一律改用系统浏览器打开，避免 WebView2 拦截新窗口后整页失去响应。
- **界面与文档**：补齐相关简体中文文案；同步 Agent 规格说明（含 Kimi、命名管道与完成闪烁约定）。

### 既有能力（仍适用于本版）

- 简体中文界面（可切换）
- Windows 终端 `Ctrl+C` / `Ctrl+V` 常用复制粘贴语义
- Claude Code / Codex / Gemini / Grok / OpenCode / Kimi 的完成与注意通知
